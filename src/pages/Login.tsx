...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const redirect = searchParams.get('redirect') || (data.user.role === 'admin' ? '/admin' : '/profile');
      navigate(redirect);
    } catch (err: any) {
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-3xl shadow-xl border border-zinc-100">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-zinc-600">
            Hoặc{' '}
            <Link to="/register" className="font-bold text-zinc-900 hover:text-zinc-700 underline underline-offset-4">
              tạo tài khoản mới
            </Link>
          </p>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <input
                ...
                name="remember-me"
                type="checkbox"
                className="h-5 w-5 text-zinc-900 focus:ring-zinc-900 border-zinc-300 rounded"
              />
              <label htmlFor="remember-me" className="ml-2 block text-sm text-zinc-900 font-medium">
                Ghi nhớ đăng nhập
              </label>
            </div>

            <div className="text-sm">
              <a href="#" className="font-bold text-zinc-900 hover:text-zinc-700 underline underline-offset-4">
                Quên mật khẩu?
              </a>
            </div>
          </div>
